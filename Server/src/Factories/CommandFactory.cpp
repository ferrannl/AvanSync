...
Factories::CommandFactory::CommandFactory(const std::shared_ptr<Controllers::MainController> main)
{
	...
	_commands.insert(std::make_pair(Enums::CommandEnum::GET_DIRECTORY_LISTING, std::make_unique<Commands::DirCommand>(main)));
	_commands.insert(std::make_pair(Enums::CommandEnum::GET_SERVER_INFO, std::make_unique<Commands::InfoCommand>(main)));
...
const std::unique_ptr<Commands::Command>& Factories::CommandFactory::get_command(Enums::CommandEnum command) const
{
